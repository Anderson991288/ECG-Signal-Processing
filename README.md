...
### 頭文件（.hea）
提供了關於數據記錄的一些基本信息，例如數據的信號數量、信號的長度、採樣率、數據存儲格式等。下面是一個示例頭文件（100.hea）的內容：
```
100.hea
2 360 650000
MLII
...
```
* 第一行表示記錄編號和擷取時間。
* 第二行中的數字 2 表示數據包含兩個信號。
* 第三行中的數字 360 表示每個信號的採樣率為 360 Hz。
* 第四行和第五行分別指定了信號的名稱（MLII和V5）。
...
```
* 每三個字節（24位）表示兩個數值，分別來自兩個信號。以示例中的數據為例，前三個字節 "E3 33 F3" 表示兩個值分別為 0x3E3 和 0x3F3。這些數值可以轉換為十進制，代表兩個信號的第一個採樣點。
